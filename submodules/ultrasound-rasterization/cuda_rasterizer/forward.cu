...
__device__ glm::vec3 computeColorFromSH(int idx, int deg, int max_coeffs, const glm::vec3* means, glm::vec3 campos, const float* shs, bool* clamped)
{
	...
	if (deg > 0)
	{
		...
		result = result - SH_C1 * y * sh[1] + SH_C1 * z * sh[2] - SH_C1 * x * sh[3];
			...
			{
				float depth = x;
				float reflection_var = x * x + y * y + z * z;
				float scattering_var_x = x * y;
				float scattering_var_y = z * y;
				float scattering_var_z = x * z;

				// Physics-driven ultrasound terms: attenuation, reflection, and scattering.
				float attenuation = exp(-depth);
				float reflection = -reflection_var;

				// Final ultrasound intensity contribution.
				result = result - attenuation * sh[4] + reflection * sh[5] - scattering_var_z * sh[6] + scattering_var_x * sh[7] - scattering_var_y * sh[8];
			}
		}
	result += 0.5f;
	...
	float* __restrict__ out_others)
{
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			// Efficient implementation of distortion loss for Gaussian splatting.
			float A = 1-T;
