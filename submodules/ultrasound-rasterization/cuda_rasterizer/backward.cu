...
__device__ void computeColorFromSH(int idx, int deg, int max_coeffs, const glm::vec3* means, glm::vec3 campos, const float* shs, const bool* clamped, const glm::vec3* dL_dcolor, glm::vec3* dL_dmeans, glm::vec3* dL_dshs)
{
	...
	if (deg > 0)
	{
		...
		dRGBdz = SH_C1 * sh[2];
			...
			{
				float depth = x;
				float dRGBdsh4 = exp(-depth);
				dL_dsh[4] = dRGBdsh4 * dL_dRGB;

				float dRGBdsh5 = x * x + y * y + z * z;
				dL_dsh[5] = dRGBdsh5 * dL_dRGB;

				// Gradients for ultrasound scattering terms.
				float dRGBdsh6 = x * y;
				float dRGBdsh7 = y * z;
				float dRGBdsh8 = x * z;
				...
				// Propagate gradients through attenuation, reflection, and scattering terms.
				dRGBdx = dRGBdx - dRGBdsh4 - dRGBdsh5 + dRGBdsh6 - dRGBdsh8;
				dRGBdy = dRGBdy - dRGBdsh5 - dRGBdsh6 + dRGBdsh7;
				dRGBdz = dRGBdz - dRGBdsh5 - dRGBdsh7 + dRGBdsh8;
			}
	}
